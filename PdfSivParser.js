...
export default class PdfSivParser extends GenericSivParser {
  ...
  async parsePdfTags() {
    try {
      ...
      for (const item of elements) {
        ...
        if (txt.includes('nomenclature')) {
        ...
        } else if (lookingForValue && !isNaN(txt)) {
          ...
          if (lookingForVariety && !country) {
            ...
            if (variety) {
              ...
              variety = this
                .varietyFromItemText(this.storage.Config.transcriptionErrors[partialVariety + txt]);
              partialVariety = '';
            ...
            } else if (/^[. 0-9]*$/.test(partialVariety + txt)) {
  ...
  static checkForPdfDate(txt) {
    ...
    // eslint-disable-next-line no-restricted-globals
    if (trio.some((a) => isNaN(a))) return undefined;
